...
- **A first screen with something to do**: An empty board opens on a Start
  ...
  *Analyze a game*; the analysis board. When a game ends, the result card
  offers *Play again* on the same terms beside the review, an empty
  analysis board offers *Import or fetch a game* where the move list will be,
  and a game that has just been brought in is offered *Review this game* on
  the tab it landed on, one press, until the review has run.
...
- **Either colour, or whichever**: The New Game dialog offered White or Black,
  ...
  move never practises the half of chess that starts a tempo down. Random is the
  third button, it is rolled when Start is pressed rather than when the board
  appears, and it stays selected for the next game: a random button that forgets
  is worse than none, because re-picking it every time is the thing you wanted
  automated.
...
- **Step through every mistake**: `‹ Previous` and `Next ›` under the review's
  ...
  decision still in it. Critical Moments ranks the five costliest and stops;
  everything past the fifth used to be reachable only by scrolling the move
  list for a coloured dot. It reads the rows the review is *reporting*, so

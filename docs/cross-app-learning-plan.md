...
| Should a mate be capped before the centipawn loss is taken? | `web-chess/docs/architecture.md` | **The clearest real bug still open.** A finished review reads "ACPL 316" beside "Mistake 0, Blunder 0", and one row says "Lost 93.61 / Inaccuracy". `scoreToCp` maps any mate to 10000, so playing into a forced mate scores as a ~9500cp loss and one move supplies most of the average. Lichess caps at ±1000cp. Not fixed because it moves a number users have seen. **xiangqi shares it** — mate maps to 30000 there rather than 10000, so one such move adds ~737 to a 40-move average; read from the code, not yet seen on screen. Pick one cap for both. |

...
| **Game report** ✅ *(partly)* | `utils/gameReport.ts`, `narrativeTags.ts`, `moveInsight.ts`, `playedMoveQuality.ts` | **chess + xiangqi.** Narrative tags, the phase split and per-phase filtering are done in both — the split reads material rather than move number, which is where katrain was the wrong model. `moveInsight` and `playedMoveQuality` are not ported. |
| **Real library** ✅ *(chess)* | `utils/library.ts`, `libraryZip.ts`, `libraryImportValidation.ts`, `libraryTextImport.ts`, `libraryKeyboard.ts` | **chess** — done: IndexedDB store with a localStorage→memory fallback, search, sort, star, rename, delete, JSON backup import/export, paged at 100 rows. Folders and zip are not ported. **xiangqi** — still a flat list; it gained paging, not folders. |
| **Auto-save + crash recovery** ✅ | `utils/autoSave.ts`, `AutoSaveRecoveryModal.tsx` | **chess + xiangqi.** Done in both, with a recovery prompt. Both learned the same lesson twice: the snapshot must depend on the evaluations too, or the engine starves the save. |
...
| **Docs set + competitive practice** ✅ *(partly)* | `docs/{architecture,engine,development,deployment,diagram}.md` and `docs/competitor-analysis.md` | **chess + xiangqi.** Both now have an `architecture.md` carrying their invariants and their known hazards. The rest of the set, and `competitor-analysis.md`, are not ported. |
...
| **Performance regression gates** ✅ *(partly)* | `scripts/bench-nps.cjs` with `--compare`, `--min-median-nps`, `--max-median-wall-ms`, `--max-cold-start-ms` | **katrain.** Done: `test/enginePerfBench.test.ts` writes JSON via `BENCH_OUT`, compares against a `BENCH_BASELINE`, and can gate on `BENCH_MAX_SEARCH_MS`; `npm run bench` runs it. Its first bug was that it printed nothing — Vitest hides console output from passing tests, so the numbers had never been seen. Not wired into CI. |
